...
// Pastikan import diarahkan ke package events
...
class EventViewModel @Inject constructor(
    ...
    fun postEvent(
        ...
        viewModelScope.launch {
            _uiState.update {
                it.copy(eventAdd = EventActionUIState.Loading)
            }
            _uiState.update { state ->
                val tmpState = runCatching {
                    ...
                    onSuccess = { response ->
                        if (response.status == "success") {
                            EventActionUIState.Success(response.message)
                        } else {
                            EventActionUIState.Error(response.message)
                        }
                    },
                    onFailure = { error ->
                        EventActionUIState.Error(error.message ?: "Unknown error")
                    }
                )

                state.copy(eventAdd = tmpState)
            }
        }
    }
    ...
    fun getEventById(authToken: String, eventId: String) {
        viewModelScope.launch {
            _uiState.update {
                it.copy(event = EventUIState.Loading)
            }
            _uiState.update { state ->
                val tmpState = runCatching {
                    ...
                    onSuccess = { response ->
                        if (response.status == "success") {
                            EventUIState.Success(response.data!!.event)
                        } else {
                            EventUIState.Error(response.message)
                        }
                    },
                    onFailure = { error ->
                        EventUIState.Error(error.message ?: "Unknown error")
                    }
                )

                state.copy(event = tmpState)
            }
        }
    }
    ...
    fun putEvent(
        ...
        viewModelScope.launch {
            _uiState.update {
                it.copy(eventChange = EventActionUIState.Loading)
            }
            _uiState.update { state ->
                val tmpState = runCatching {
                    repository.putEvent(
                ...
                state.copy(eventChange = tmpState)
            }
        }
    }
    ...
    fun putEventCover(
        ...
        viewModelScope.launch {
            _uiState.update {
                it.copy(eventChangeCover = EventActionUIState.Loading)
            }
            _uiState.update { state ->
                val tmpState = runCatching {
                    repository.putEventCover(
                        authToken = authToken,
                        eventId = eventId,
                        file = file
                    )
                ...
                state.copy(eventChangeCover = tmpState)
            }
        }
    }
    ...
    fun deleteEvent(authToken: String, eventId: String) {
        viewModelScope.launch {
            _uiState.update {
                it.copy(eventDelete = EventActionUIState.Loading)
            }
            _uiState.update { state ->
                val tmpState = runCatching {
                ...
                state.copy(eventDelete = tmpState)
            }
        }
    }
